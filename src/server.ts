...
app.use((req, res, next) => {
  if (!config.LOCAL_API_KEY) return next();
  const token = req.header('Authorization')?.replace(/^Bearer\s+/i, '');
  if (req.path === '/health') return next();
  if (token !== config.LOCAL_API_KEY) return res.status(401).json({ error: { message: 'Unauthorized' } });
  next();
});
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    upstream.headers.forEach((value, key) => {
      if (['content-type', 'content-length'].includes(key.toLowerCase())) res.setHeader(key, value);
    });
    if (upstream.body) {
